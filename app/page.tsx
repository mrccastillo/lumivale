    ...
    <div className="bg-[#f7f8fb] text-[var(--lumivale-ink)]">
      <div data-nav-surface="dark" className="relative isolate overflow-hidden bg-[radial-gradient(circle_at_50%_0%,rgba(20,201,131,0.26),transparent_26%),radial-gradient(circle_at_50%_44%,rgba(20,201,131,0.12),transparent_30%),linear-gradient(180deg,#063322_0%,#031410_48%,#031410_74%,#010807_100%)] text-white">
        <section id="hero" data-theme="dark" className="px-4 pb-6 pt-[72px] sm:px-6 sm:pb-8 sm:pt-20">
          <div className="mx-auto flex min-h-[52vh] max-w-7xl flex-col items-center justify-center text-center sm:min-h-[56vh]">
            ...
            <div data-testid="platform-row" className="mt-8 flex max-w-full flex-nowrap items-center justify-center gap-x-4 text-sm font-semibold text-white/56 sm:mt-10 sm:max-w-none sm:gap-x-12 sm:text-xl">
        ...
        <section id="proof" className="px-4 pb-14 pt-0 text-white sm:px-6 sm:pb-[68px]">
          <div className="mx-auto max-w-7xl text-center">
            ...
            <div className="mx-auto mt-8 grid max-w-5xl overflow-hidden rounded-lg border border-white/10 bg-white/[0.03] sm:mt-12 md:grid-cols-3">
              {metrics.map((metric) => (
                <article key={metric.value} className="border-white/10 p-6 text-left sm:p-8 md:border-r last:border-r-0">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-5 sm:mt-14 md:grid-cols-2 xl:grid-cols-3">
            {services.map((service) => (
              <article
                key={service.slug}
                className="flex min-h-[220px] flex-col rounded-lg border border-[var(--lumivale-line)] bg-[#fbfcff] p-6 shadow-[0_20px_60px_rgba(42,47,82,0.06)] transition hover:-translate-y-1 hover:border-[var(--lumivale-accent)] hover:shadow-[0_24px_70px_rgba(42,47,82,0.1)] sm:p-7"
              >
          ...
          {testimonials.length ? (
            <div className="mt-8 grid gap-5 md:grid-cols-2">
              {testimonials.map((testimonial) => (
                <article
                  key={testimonial.id}
                  className="flex min-h-[260px] flex-col justify-between rounded-lg border border-white/10 bg-white/[0.04] p-6 text-left sm:p-7"
                >
                  {testimonial.type === "video" && testimonial.videoFileId ? (
                    <video
                      controls
                      preload="metadata"
                      src={`/api/testimonial-videos/${testimonial.videoFileId}`}
                      className="mb-5 aspect-video w-full rounded-lg bg-black object-cover"
                    />
                  ) : null}
                  <blockquote className="text-xl font-semibold leading-tight sm:text-2xl">
                    {testimonial.quote}
                  </blockquote>
                  <div className="mt-6">
                    <p className="font-semibold text-white">{testimonial.personName}</p>
                    {testimonial.personTitle ? (
                      <p className="mt-1 text-sm text-[#b9d9c8]">
                        {testimonial.personTitle}
                      </p>
                    ) : null}
                  </div>
            ...
            <div className="text-center">
              <blockquote className="mt-5 text-2xl font-semibold leading-tight sm:mt-6 sm:text-4xl">
      ...
      <section id="faqs" className="bg-white px-4 py-16 sm:px-6 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 sm:gap-12 lg:grid-cols-[0.74fr_1.26fr] lg:gap-20">
